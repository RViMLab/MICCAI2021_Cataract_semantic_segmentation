...
class LovaszSoftmax(nn.Module):
    def __init__(self, config):
        ...
        self.classes_to_ignore = None if 'classes_to_ignore' not in config else config['classes_to_ignore']
    ...
    def lovasz_softmax_flat(self, prob: torch.Tensor, lbl: torch.Tensor) -> torch.Tensor:
        ...
        if self.experiment in [2, 3] and c in class_to_sum:  # Experiments 2 and 3: ignore the last ('ignore') class
            class_to_sum.remove(c)
