    ...
    def train_one_epoch(self):
        ...
        for batch_num, (img, lbl, metadata) in enumerate(self.data_loaders[self.train_schedule[self.epoch]]):
            ...
            if self.model.projector_model and isinstance(self.loss, LossWrapper):
                interm_output, output, proj_features = self.model(img.float())
                loss = self.loss(proj_features, output, lbl.long(), interm_prediction=interm_output)
            else:
                interm_output, output = self.model(img.float())
                loss = self.loss(interm_output, output, lbl.long())
            # backward
            loss.backward()
            self.optimiser.step()
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for rec_num, (img, lbl, metadata) in enumerate(self.data_loaders['valid_loader']):
                ...
                if self.model.projector_model and isinstance(self.loss, LossWrapper):
                    interm_output, output, proj_features = self.model(img.float())
                    valid_loss += self.loss(proj_features, output, lbl.long(), interm_prediction=interm_output).item()
                    print(valid_loss) if self.debugging else None
                    for key in self.loss.loss_vals:
                        individual_losses[key] += self.loss.loss_vals[key]
                else:
                    interm_output, output = self.model(img.float())
                    valid_loss += self.loss(interm_output, output, lbl.long()).item()
                    print(valid_loss) if self.debugging else None
                ...
                # break
        # self.metrics['best_miou_epoch_step'] = (1, 1)
